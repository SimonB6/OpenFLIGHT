...
template<typename T, typename I, typename I2>
void exponent(const TPC<T, I> &in, TPC<T, I2> &out) {
    /*
     * Approximation:
     *   > exponent(x) = (1 + \frac{x}{2^t})^{2^t}, empirically t = 5.
     * WARNING: Only works for x \in [T_\text{exp}, 0].
     */
    size_t size = in.size();
    TPC<T> temp(size);
    temp += in;
    dividePublic(temp, (T)1 << 5);
    temp += 1;
...
template<typename T, typename I, typename I2>
void softmax(const TPC<T, I> &in, TPC<T, I2> &out) {
    /*
     * Compute exp / sum exp.
     */
    size_t in_size = in.size();
    size_t out_size = out.size();
    size_t k = in_size / out_size;
    assert(k * out_size == in_size && "TPC softmax input size mismatch");

    TPC<T> x(in_size), xhat(out_size);
    TPC<uint8_t> dReLU(in_size);
    x += in;
    maxpool(x, xhat, dReLU, k);
    x -= xhat;
}
...
template<typename T, typename I, typename I2>
void scan(TPC<T, I> &input, TPC<T, I2> &result, int k) {
...
// TODO: Not implement.
template<typename T, typename I, typename I2>
void multn(const TPC<T, I> &input, TPC<T, I2> &output, size_t n, uint64_t prime)
{
    assert(input.size() == (output.size() * n) && "multn input/output size mismatch");
    size_t size = input.size() / 2;
    size_t redundancy = input.size() - size * 2;
    ...
    while (size > 1)
    {
        ...
        a *= b;
        a %= prime;
        size = (size + redundancy) / 2;
        redundancy = a.size() + redundancy - size * 2;
    }
...
template<typename T, typename I, typename I2>
void square(const TPC<T, I> &in, TPC<T, I2> &out) {

    size_t size = in.size();
    TPC<T> x(size), z(size);
    x.fill(0), z.fill(0);
    DeviceData<T> e(size), temp(size);

    *x.getShare(0) += *in.getShare(0); 
    reconstruct(x, e); 
    *x.getShare(0) -= *in.getShare(0);
